...
- **News datasets** (cột `source`, schema chung): `cafef_articles.csv` ~3.348 (daily RSS tích lũy; **backfill sâu bị cafef throttle ở mọi concurrency** — workers=4 vẫn 0 kept — nên defer, dựa vào daily RSS), `ssi_articles.csv` ~1.860 (đã đủ ~217 trang), `hsc_articles.csv` ~6 (HSC ít + **không pub_date**), `vndirect_articles.csv` đang backfill (Playwright-stealth, 4 category).
